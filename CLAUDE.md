...
- **Paragraph content interleaving**: Runs, comment ranges, bookmarks, and SDTs must maintain their original order within a paragraph. `Paragraph.OrderedContent` preserves this interleaving from parsed documents. Splitting into separate `Runs`/`RawXMLElements` lists and emitting runs-first breaks comment range references and causes "unreadable content" dialogs. Programmatically-created paragraphs (no `OrderedContent`) fall back to the runs-then-raw order.
- **Template document.xml.rels preservation**: For opened templates, the original `word/_rels/document.xml.rels` is preserved verbatim (`originalDocRelsData`) and only new relationships are appended. Regenerating from scratch created duplicate rId entries which triggered "unreadable content" dialogs. New documents (no template) still build rels from scratch with styles.xml as rId1.
